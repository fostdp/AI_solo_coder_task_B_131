const EraComparator = (function() {
    ...
    function destroy() {
        ...
        if (droneSel) droneSel.innerHTML = '';
        const radarCanvas = document.getElementById('crossEraRadarCanvas');
        if (radarCanvas) {
            const ctx = radarCanvas.getContext('2d');
            ctx.clearRect(0, 0, radarCanvas.width, radarCanvas.height);
        }
        const barCanvas = document.getElementById('crossEraBarCanvas');
        if (barCanvas) {
            const ctx = barCanvas.getContext('2d');
            ctx.clearRect(0, 0, barCanvas.width, barCanvas.height);
        }
        const tableContainer = document.getElementById('crossEraTableContainer');
    ...
    async function loadComparison() {
        try {
            const cartSel = document.getElementById('cartSelectCross');
            const droneSel = document.getElementById('droneSelectCross');
            let url = meta.apiBase + '/cross-era';
            if (cartSel?.value && droneSel?.value) {
                url = meta.apiBase + `/vs-drone?cartId=${cartSel.value}&droneId=${droneSel.value}`;
            }
