const CoverageOptimizer = (function() {
    const meta = {
    ...
    function destroy() {
        ...
        initialized = false;
        const btn = document.getElementById('runCollab');
        if (btn) btn.removeEventListener('click', runOptimization);
        const canvas = document.getElementById('collabCoverageCanvas');
        if (canvas) {
            const ctx = canvas.getContext('2d');
            ctx.clearRect(0, 0, canvas.width, canvas.height);
        }
        const metricsEl = document.getElementById('collabMetrics');
        if (metricsEl) metricsEl.innerHTML = '';
        const blindZonesEl = document.getElementById('collabBlindZones');
        if (blindZonesEl) blindZonesEl.innerHTML = '';
    }
    ...
    async function runOptimization() {
        ...
        try {
            const res = await fetch(meta.apiBase + '/optimize', {
                method: 'POST',
