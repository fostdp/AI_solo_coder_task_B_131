const EvolutionAnalyzer = (function() {
    const meta = {
    ...
    async function init() {
        ...
        try {
            const [cartsRes, evoRes] = await Promise.all([
                fetch(meta.apiBase + '/carts'),
                fetch(meta.apiBase + '/evolution')
            ]);
    ...
    function destroy() {
        ...
        evolutionData = null;
        initialized = false;
        const timelineCanvas = document.getElementById('dynastyTimelineCanvas');
        if (timelineCanvas) {
            const ctx = timelineCanvas.getContext('2d');
            ctx.clearRect(0, 0, timelineCanvas.width, timelineCanvas.height);
        }
        const radarCanvas = document.getElementById('dynastyRadarCanvas');
        if (radarCanvas) {
            const ctx = radarCanvas.getContext('2d');
            ctx.clearRect(0, 0, radarCanvas.width, radarCanvas.height);
        }
        const tableContainer = document.getElementById('dynastyTableContainer');
        if (tableContainer) tableContainer.innerHTML = '';
        const insightsEl = document.getElementById('dynastyInsights');
    ...
    async function refresh() {
        ...
        try {
            ...
            console.error('Failed to refresh dynasty data', e);
        }
    }
